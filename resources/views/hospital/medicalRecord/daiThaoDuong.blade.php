...
@section('content')
    <div>
        <form action="hospital/medicalRecord/them-{{"bệnh đái tháo đường"}}" method="POST">
            ...
            <div id="chuanDoan">
                <div id="page-wrapper">
                    ...
                    @foreach($question['xetNghiem']['thuongQuy'] as $i => $que)
                        ...
                        <label class="radio-inline">
                            <input name="thuongQuy{{$i}}" value="Co" type="radio">Có
                        </label>
                        <label class="radio-inline">
                            <input name="thuongQuy{{$i}}" value="Khong" type="radio">Không
                        </label>
                        <label class="radio-inline">
                    ...
                    <h3>V.CHUẨN ĐOÁN</h3>
                    @foreach($question['chuanDoan'] as $i => $que)
                        <label>21.	Đo đường máu (glucose máu (GM) lúc đói đo 2 lần)</label>
                        <p><strong>{{$que['_id']}}.{{$que['text']}}</strong></p>
                        <label class="radio-inline">
                            <input name="chuanDoan{{$i}}" value="Co" type="radio">Có
                        </label>
                        <label class="radio-inline">
                            <input name="chuanDoan{{$i}}" value="Khong" type="radio">Không
                        </label>
                        <label class="radio-inline">
                            <input name="chuanDoan{{$i}}" value="ChuaChon" type="radio">Chưa chọn
                        </label>
                    @endforeach
                    <br>

                    <h3>VI.  XÉT NGHIỆM CẬN LÂM SÀNG ĐỂ THEO DÕI ĐIỀU TRỊ</h3>
                    <textarea class="form-control" style="height: 100px; width: 500px;" name="canLs"></textarea>

                    <h3>VII. TRIỆU CHỨNG LÂM SÀNG VÀ XÉT NGHIỆM ĐẶC BIỆT PHÁT HIỆN BIẾN CHỨNG ĐÁI THÁO ĐƯỜNG</h3>
                    <h4>1.	Biến chứng cấp tính do ĐTĐ giai đoạn đầu:</h4>
                    @foreach($question['xetNghiemDacBiet']['bcCapTinh'] as $i => $que)
                        <p><strong>{{$que['_id']}}.{{$que['text']}}</strong></p>
                        <label class="radio-inline">
                            <input name="bcCapTinh{{$i}}" value="Co" type="radio">Có
                        </label>
                        <label class="radio-inline">
                            <input name="bcCapTinh{{$i}}" value="Khong" type="radio">Không
                        </label>
                        <label class="radio-inline">
                            <input name="bcCapTinh{{$i}}" value="ChuaChon" type="radio">Chưa chọn
                        </label>
                    @endforeach

                    <h4>2.	Biến chứng mạch máu lớn ( biến chứng mạn tính ĐTĐ):</h4>
                    @foreach($question['xetNghiemDacBiet']['bcMachMauLon'] as $i => $que)
                        <p><strong>{{$que['_id']}}.{{$que['text']}}</strong></p>
                        <label class="radio-inline">
                            <input name="bcMachMauLon{{$i}}" value="Co" type="radio">Có
                        </label>
                        <label class="radio-inline">
                            <input name="bcMachMauLon{{$i}}" value="Khong" type="radio">Không
                        </label>
                        <label class="radio-inline">
                            <input name="bcMachMauLon{{$i}}" value="ChuaChon" type="radio">Chưa chọn
                        </label>
                    @endforeach

                    <h4>3.	Biến chứng mạch máu nhỏ do ĐTĐ (b/c mãn tính)</h4>
                    @foreach($question['xetNghiemDacBiet']['bcMachMauNho'] as $i => $que)
                        <p><strong>{{$que['_id']}}.{{$que['text']}}</strong></p>
                        <label class="radio-inline">
                            <input name="bcMachMauNho{{$i}}" value="Co" type="radio">Có
                        </label>
                        <label class="radio-inline">
                            <input name="bcMachMauNho{{$i}}" value="Khong" type="radio">Không
                        </label>
                        <label class="radio-inline">
                            <input name="bcMachMauNho{{$i}}" value="ChuaChon" type="radio">Chưa chọn
                        </label>
                    @endforeach
